...
// This component simulates the SentenceBox dependency.
const SentenceBox = memo(({ sentence, onClear, onBackspace }) => {
    ...
    return (
        <div className="mt-8 p-4 bg-gray-50 rounded-xl shadow-inner border border-gray-200 animate-fadeIn">
            <h3 className="text-sm font-semibold text-gray-500 mb-3 uppercase tracking-wider">Current Sentence</h3>
            <div className="p-4 mb-4 bg-white border border-indigo-200 rounded-lg min-h-[4rem] flex items-center shadow-sm">
                <p className="font-medium text-gray-800 break-words text-lg">
                    {sentence || <span className="text-gray-400 italic">Upload an image to start building your sentence...</span>}
                </p>
            </div>
            {sentence && (
                <div className="flex flex-wrap justify-end gap-3 mt-4">
                    <button
                        className="flex-1 min-w-[12rem] m-5 px-4 py-3 text-sm font-extrabold text-black bg-green-600 rounded-10 shadow-lg hover:bg-green-700 transition duration-300 transform hover:scale-[1.02] disabled:opacity-50"
                        onClick={() => handleRead(sentence)}
                        disabled={!sentence.trim()}
                    >
                        🔊 Read Full Word
                    </button>
                    {/* Clear Button - Fixed width for consistent layout */}
                    <button
                        className="w-full sm:w-24 px-4 py-3 text-sm font-extrabold text-white bg-red-600 rounded-xl shadow-lg hover:bg-red-700 transition duration-300 transform hover:scale-[1.02] disabled:opacity-50"
                        onClick={onClear}
                        disabled={!sentence}
                    >
                        Clear
                    </button>
                    {/* Backspace Button - Fixed width for consistent layout */}
                    <button
                        className="w-full sm:w-28 px-4 py-3 text-sm font-extrabold text-gray-800 bg-yellow-400 rounded-xl shadow-lg hover:bg-yellow-500 transition duration-300 transform hover:scale-[1.02] disabled:opacity-50"
                        onClick={onBackspace}
                        disabled={!sentence}
                    >
                        ⌫ Backspace
    ...
    return (
        <div className="app-container">          
            ...
            <div className="main-card">
                ...
                <button
                    onClick={goBack}
                    className="w-full btn-base btn-secondary-back mt-10 btn-hover"
                    type="button"
                >
                    ⬅ Back to Home
                </button>
